...
import { Trophy, ArrowLeft, Phone, CheckCircle, Smartphone, ShieldCheck, Clock, User as UserIcon, MapPin, Key, Lock, Mail } from './Icons';

type AuthView = "login" | "register" | "otp_verify" | "forgot_pin" | "reset_pin_entry";
...
export default function Auth() {
  ...
  const startRegistration = async () => {
    if (!regData.name || mobile.length !== 10 || !pin) return handleError("All * fields are required");
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#050505] p-4 font-chakra overflow-hidden">
      <div className="w-full max-w-sm">
        ...
        <div className="text-center mb-10 animate-fade-in">
          ...
          <p className="text-[9px] text-gray-500 font-bold uppercase tracking-[0.5em] mt-1">Next-Gen Earning Portal</p>
        </div>
        ...
        <div className="bg-[#111] border border-gray-800 rounded-3xl p-8 shadow-[0_20px_60px_rgba(0,0,0,0.9)] relative overflow-hidden">
          ...
          <h2 className="text-white text-xl font-black mb-8 text-center uppercase tracking-tight flex items-center justify-center gap-2">
            {view === "login" && <><ShieldCheck size={20} className="text-yellow-500"/> SYSTEM LOGIN</>}
          ...
          <div className="space-y-4">
            ...
            {view === "login" && (
              <div className="space-y-4 animate-fade-in">
                ...
                <div className="flex justify-between px-1">
                   ...
                   <button onClick={() => setView("register")} className="text-[10px] text-yellow-600 font-bold uppercase hover:text-white">Create New Account</button>
                </div>
              </div>
            )}
            ...
            {view === "register" && (
              <div className="space-y-3 animate-fade-in max-h-[60vh] overflow-y-auto no-scrollbar pr-1">
                ...
                <InputGroup icon={<Lock size={18}/>}>
                  <input type="password" placeholder="Create PIN *" className="auth-input" value={pin} onChange={e => setPin(e.target.value)}/>
                </InputGroup>
            ...
            {view === "otp_verify" && (
              <div className="space-y-6 animate-fade-in text-center">
                ...
                <div className="flex justify-between items-center px-1">
                   <button onClick={() => setView("register")} className="text-[10px] text-gray-500 uppercase font-bold">Edit Details</button>
            ...
            {view === "forgot_pin" && (
              <div className="space-y-6 animate-fade-in">
                <p className="text-[10px] text-gray-500 uppercase text-center tracking-wider">Enter your registered mobile number</p>
                <InputGroup icon={<Phone size={18}/>} prefix="+91">
                  <input type="tel" maxLength={10} placeholder="Mobile Number" className="auth-input" value={mobile} onChange={e => setMobile(e.target.value.replace(/\D/g, ''))}/>
                </InputGroup>
        ...
        <div className="mt-8 text-center text-[9px] text-gray-800 font-bold tracking-[0.3em] uppercase opacity-40">
          Broo Connect • Secure Auth v4.0.0
        </div>
      </div>
...
// Fixed InputGroup by ensuring the React namespace is available via import
