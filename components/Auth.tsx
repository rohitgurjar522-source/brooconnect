...
export default function Auth() {
  ...
  const finalizeAuth = async (verifiedMobile: string) => {
    try {
      // Normalize mobile for database (ensure 91 prefix)
      const dbMobile = verifiedMobile.startsWith('+') ? verifiedMobile.slice(1) : verifiedMobile;
      ...
      alert("Account synchronization failed: " + err.message);
      setLoading(false);
    }
  };
  ...
  const handleSendOTP = () => {
    ...
    if (window.sendOtp) {
      ...
      alert("OTP System initialization pending. Please refresh the page.");
      setLoading(false);
    }
  };
  ...
  const handleVerifyOTP = async () => {
    ...
    setLoading(true);
    try {
      const identifier = "91" + mobile; // API expects format without '+' sometimes or with, matching what was sent
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#050505] p-4 font-chakra">
      <div className="w-full max-w-sm">
        ...
        <div className="text-center mb-10 animate-fade-in">
          ...
          <p className="text-[9px] text-gray-500 font-bold uppercase tracking-[0.5em] mt-1">Premium Earning Ecosystem</p>
        </div>
        ...
        <div className="bg-[#111] border border-gray-800 rounded-3xl p-8 shadow-[0_20px_60px_rgba(0,0,0,0.9)] relative overflow-hidden">
          ...
          <h2 className="text-white text-xl font-black mb-8 text-center uppercase tracking-tight flex items-center justify-center gap-2">
            {!otpSent ? <><ShieldCheck size={20} className="text-yellow-500"/> SECURE LOGIN</> : <><Smartphone size={20} className="text-yellow-500"/> VERIFY MOBILE</>}
          </h2>
          ...
          <div className="space-y-6">
            {!otpSent ? (
              <>
                ...
                <p className="text-center text-[10px] text-gray-500 font-medium uppercase tracking-wider leading-relaxed">
                  OTP will be sent to your mobile for secure authentication.
                </p>

                <button 
                  onClick={handleSendOTP} 
                  disabled={loading} 
                  className="w-full btn-gaming py-4 rounded-2xl text-black font-black uppercase tracking-wider active:scale-95 transition-all text-sm flex items-center justify-center gap-2 shadow-[0_10px_20px_rgba(234,179,8,0.15)]"
                >
                  {loading ? (
                    <div className="w-5 h-5 border-2 border-black border-t-transparent rounded-full animate-spin"></div>
                  ) : (
                    <>GET ACCESS CODE <Smartphone size={18}/></>
                  )}
                </button>
              </>
            ) : (
              <>
                ...
                <div className="text-center">
                  ...
                  <div className="flex justify-center gap-4">
                    <button onClick={() => { setOtpSent(false); setOtp(""); }} className="text-[10px] text-gray-500 font-bold uppercase hover:text-white flex items-center gap-1">
            ...
            <div className="pt-6 text-center border-t border-gray-800/50">
               <div className="flex items-center justify-center gap-2 mb-2">
                 ...
                 <span className="text-[9px] text-gray-500 font-bold uppercase tracking-widest">Encrypted Server Connection</span>
               </div>
               <p className="text-[8px] text-gray-600 leading-relaxed max-w-[240px] mx-auto uppercase">
                 By logging in, you agree to our terms. Security handled by MSG91 Enterprise.
               </p>
            </div>
          </div>
        </div>
        ...
        {/* Footer */}
        <div className="mt-8 text-center text-[9px] text-gray-800 font-bold tracking-[0.3em] uppercase opacity-50">
          Broo Connect System v2.7.0 • Stable
        </div>
      </div>
    </div>
  );
